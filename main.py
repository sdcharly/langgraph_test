...
def process_input(user_input):
    # Prepare the initial message with user input
    initial_message = HumanMessage(content=user_input)

    # Run the graph with the initial message
    for state in graph.stream({"messages": [initial_message]}):
        if "__end__" not in state:
            # Process and collect the output from each state
            # Assuming the final response you need is in 'state'
            # Modify this part as per your graph's output structure
            final_response = state.get('messages', [])
            if final_response:
                return final_response[-1].content  # Return the content of the last message

    # Fallback in case no response is generated
    return "No response generated by the graph"
