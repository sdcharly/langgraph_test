...
def process_input(user_input):
    try:
        app.logger.info("Starting graph processing for input: {}".format(user_input))
        
        response = graph.stream({"messages": [HumanMessage(content=user_input)]})
        app.logger.info("Graph stream response: {}".format(response))

        final_state = next((state for state in response if "__end__" in state), None)
        
        if final_state and 'messages' in final_state:
            final_message = final_state['messages'][-1]
            if isinstance(final_message, HumanMessage):
                app.logger.info("Final graph output: {}".format(final_message.content))
                return final_message.content
            else:
                app.logger.warning("Unexpected message format in final state")
                return "Unexpected message format"
        else:
            app.logger.warning("No valid response generated by the graph")
            return "No valid response generated by the graph"
    except Exception as e:
        app.logger.error("Error during graph processing: {}".format(str(e)))
        return "Error during processing: {}".format(str(e))
